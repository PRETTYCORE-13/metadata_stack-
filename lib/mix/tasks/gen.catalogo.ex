defmodule Mix.Tasks.Gen.Catalogo do
  ...
  @moduledoc """
  Uso: mix gen.catalogo <schema_nombre>
  ...
  def run([schema_nombre]) do
    Mix.Task.run("app.config")

    {:ok, resultado, _apps} =
      Ecto.Migrator.with_repo(MetadataApp.Repo, fn _repo ->
        MetadataApp.CatalogoGenerador.generar(schema_nombre)
      end)
  ...
  def run(_args) do
    Mix.raise("Uso: mix gen.catalogo <schema_nombre>  (ej. mix gen.catalogo talla)")
  end
end
