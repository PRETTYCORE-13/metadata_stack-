defmodule MetadataApp.BCCliente do
  @moduledoc """
  Única puerta de entrada para que una regla de negocio (`StateEngine.
  ReglaPre`/`ReglaPost`) interactúe con OTRO Business Context. Una regla
  nunca debe importar `CatalogoGenerico`/`StateEngine`/`MetaSchemaContext`
  ...
  alias MetadataApp.{CatalogoGenerico, MetaSchemaContext, StateEngine}
  ...
  llamada devuelve el mismo error estructurado que `StateEngine.
  ...
  def ejecutar_transicion(tabla, id, accion, contexto \\ %{}) do
    with {:ok, registro} <- obtener(tabla, id) do
      StateEngine.ejecutar_transicion(registro, accion, contexto)
    end
  end
