defmodule MetadataAppWeb.TransicionReglaController do
  @moduledoc """
  ...
  `MetadataApp.StateEngine.Reglas`).
  """
  ...
  def index(conn, %{"transicion_id" => transicion_id}) do
    data = transicion_id |> MotorEstadosAdmin.listar_reglas() |> Enum.map(&regla_data/1)
    json(conn, %{data: data})
  end

  def create(conn, %{"meta_schema_transicion_regla" => attrs}) when is_list(attrs) do
    with {:ok, reglas} <- MotorEstadosAdmin.crear_reglas(attrs) do
      conn
      |> put_status(:created)
  ...
  def create(conn, %{"meta_schema_transicion_regla" => attrs}) do
    with {:ok, regla} <- MotorEstadosAdmin.crear_regla(attrs) do
      conn
      |> put_status(:created)
