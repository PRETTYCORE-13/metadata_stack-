defmodule MetadataAppWeb.TransicionController do
  @moduledoc """
  ...
  - `ejecutar/2`: Contrato 2 (ejecución) — corre
    `MetadataApp.StateEngine.ejecutar_transicion/3`; los desenlaces de error
  ...
  def ejecutar(conn, %{"tabla" => tabla, "id" => id, "accion" => accion} = params) do
    with {:ok, schema_mod} <- resolver(tabla) do
      ...
      with {:ok, actualizado} <- StateEngine.ejecutar_transicion(registro, accion, contexto) do
