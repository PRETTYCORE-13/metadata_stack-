defmodule MetadataAppWeb.TransicionAdminController do
  @moduledoc """
  ...
  registro real es responsabilidad de `MetadataAppWeb.TransicionController`.
  """
  ...
  def index(conn, %{"meta_schema_header_id" => header_id}) do
    data = header_id |> MotorEstadosAdmin.listar_transiciones() |> Enum.map(&transicion_data/1)
    json(conn, %{data: data})
  end

  def create(conn, %{"meta_schema_transicion" => attrs}) when is_list(attrs) do
    with {:ok, transiciones} <- MotorEstadosAdmin.crear_transiciones(attrs) do
      conn
      |> put_status(:created)
  ...
  def create(conn, %{"meta_schema_transicion" => attrs}) do
    with {:ok, transicion} <- MotorEstadosAdmin.crear_transicion(attrs) do
      conn
      |> put_status(:created)
