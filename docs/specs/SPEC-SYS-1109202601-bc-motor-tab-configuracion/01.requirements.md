...
**Documento:** Requirements · **Fase:** ✅ aprobada (2026-09-11) — documentación retroactiva. Se actualiza acá primero cuando haya un cambio real que planear sobre el tab. · ✅ §2.1 "Filtros fijos" (R24-R30) aprobada e implementada (2026-09-24). · ✅ §2.2 "Filtrar por diccionario" (R31-R34) aprobada (2026-09-25), junto con SPEC-SYS-2509202601.
...
## 2.2 Filtrar por diccionario (2026-09-25, a pedido explícito)

Lado del campo referencia de SPEC-SYS-2509202601 (SQL View de uso
Diccionario). Las reglas de fondo (qué Diccionarios se ofrecen, contrato
`id` + descripción, validación al guardar) viven en esa spec, R20-R28;
aquí solo lo que se ve y se configura en el formulario "Filtros".

R31. EL formulario "Filtros" de un campo referencia DEBE tener una
tercera sección, **"Filtrar por diccionario"**, con un selector que solo
lista las SQL View de uso Diccionario, no visibles y que tengan
autorizado el BC del campo (SPEC-SYS-2509202601 R24).

R32. CUANDO se elige un Diccionario, EL formulario DEBE pedir qué
columnas del Diccionario forman la descripción del combo (al menos una),
y al guardar verificar que todos sus `id` existen en el catálogo destino
(SPEC-SYS-2509202601 R25).

R33. CUANDO el campo tiene Diccionario, cada filtro fijo y cada
dependencia DEBE poder indicar si aplica sobre el **catálogo destino**
(como hasta ahora) o sobre una **columna del Diccionario**
(SPEC-SYS-2509202601 R27). Por defecto, sobre el destino, así que la
configuración que ya existe no cambia.

R34. EL botón "Filtros ✓" de R7 DEBE contar también un Diccionario
configurado.
