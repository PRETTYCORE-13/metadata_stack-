...
**Documento:** Tasks · **Fase:** ✅ Grupos A-H completos (2026-09-21) — Grupo H con salvedad, ver tarea 38 (repetición real completa pendiente cuando `testing` esté sano de nuevo, `docs/roadmap.md` #17).
...
38. [x] **Verificación real — encontró y corrigió 2 bugs reales, un
    ...
    - **Hallazgo 3 (aparte, no de esta spec, ver `docs/roadmap.md` #17)**:
      el segundo intento de verificación end-to-end contra `testing`
      (con los 2 fixes ya aplicados) no pudo completarse — el deploy
      falló migrando una migración VIEJA no relacionada
      (`pty_dsd_mat_material` referencia `pty_dsd_mat_marca`, ausente
      en `testing`) antes de siquiera llegar a la migración de prueba.
      `testing` queda bloqueado para deploys nuevos hasta que se
      resuelva ese drift — documentado, no arreglado hoy (fuera de
      alcance de esta spec).
    - **Estado final de la verificación real**: el fix del clasificador
      (bug 1) quedó confirmado remoto, en real, dos veces. El fix del
      targeting de rollback (bug 2) quedó confirmado preciso, pero SOLO
      en local (imposible repetir el ciclo completo end-to-end en
      `testing` por el hallazgo 3) — **pendiente una repetición real
      completa el día que `testing` esté sano de nuevo**, antes de
      confiar en Grupo H contra un cliente real (`ennova`).
    - Todas las migraciones descartables usadas en la verificación se
      revirtieron a mano (nunca con el mecanismo bajo prueba, para no
      arriesgar más) y se sacaron del repo — no queda residuo.
39. [x] `mix test` completo: 699/701 (2 fallas preexistentes sin
    relación, mismas de siempre) — limpio respecto a los cambios de hoy.
40. [x] `docs/roadmap.md` #17 agregado con el hallazgo 3.
