...
`SPEC-ARQ-0309202601-alta-sistema-nuevo` ya resolvió cómo se **crea**
...
`SPEC-ARQ-0309202601` ya cerró (✅ aprobada, R1-R10) — no se reabre
...
**Actualización 2026-09-22**: esa spec futura ya existe —
`SPEC-ARQ-2209202602-propagacion-artefactos-negocio`, documentación
retroactiva del mecanismo actual (sin canal testing/stable propio,
confirmado como diseño intencional, no como algo pendiente de
construir).
...
`SPEC-ARQ-0309202601` (R10, design.md/§3) y en los comentarios del
...
(mismo mecanismo de R8/`SPEC-ARQ-0309202601`, `imagen_actual/2`) —
...
desincronice" ya establecido en R8/§3 de `SPEC-ARQ-0309202601` — nunca
...
**Bug real encontrado (2026-09-22)**: R9a se implementó shelleando los
binarios `git`/`gh` (`System.cmd/3`) — funciona corriendo `mix
phx.server` en la máquina de un desarrollador (que sí los tiene
instalados), pero la pantalla también se abre directo contra un
sistema YA DESPLEGADO (ej. `https://unstable.ventaenruta.com.mx/sysadmin/propagacion`),
donde el proceso corre DENTRO del contenedor de producción — que no
tiene `git` ni `gh` instalados (ver `Dockerfile`, imagen final). El
fallo se atrapaba en silencio (`rescue ErlangError -> []`), mostrando
"Sin commits para mostrar" sin ninguna pista de qué pasó realmente.

R9c (corrección, a pedido explícito). EL SISTEMA DEBE obtener los
commits de `origin/main` y el historial de runs de
`actualizar-sistema.yml` vía la API HTTP de GitHub (autenticada con un
token de solo lectura), nunca shelleando `git`/`gh` — así la consulta
funciona igual corriendo dentro de un pod desplegado que en la máquina
de un desarrollador. CUANDO el token no está configurado en ese
ambiente, EL SISTEMA DEBE mostrar un error explícito indicando qué
falta — nunca una lista vacía silenciosa como hasta ahora.

**Fuera de alcance de esta corrección (decisión confirmada con el
usuario, 2026-09-22)**: los botones "Propagar..."/"rollback" de esta
misma pantalla (R8/R10) también shellean `gh` (`MotorAlta.tags_del_registro/0`,
`MotorAlta.disparar_actualizacion_sin_guarda/2`) y por lo tanto
**siguen sin funcionar** abiertos desde un pod desplegado — arreglarlo
necesitaría un token con permiso de disparar workflows viviendo en
producción, un tradeoff de seguridad mayor que el de R9c (token de
solo lectura). Mientras tanto, disparar una propagación real sigue
requiriendo `mix motor.propagar_extension` por terminal, o abrir la
pantalla desde una máquina de desarrollador (no desde la URL del
sistema desplegado).
...
- Todo lo ya resuelto por `SPEC-ARQ-0309202601` (cómo se da de alta un
