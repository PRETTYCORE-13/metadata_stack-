...
Extiende el mecanismo de `SPEC-ARQ-0309202601-alta-sistema-nuevo`
...
Referencias a actualizar (R3): `SPEC-ARQ-0309202601/requirements.md`
R10, su `design.md` §3, y el comentario de
`AmbientesLive`/`meta_publicador.ex` si mencionan el nombre viejo
(confirmar con grep al ejecutar `03.tasks.md`, no listar acá para no
...
- **Nada de esto se guarda en una tabla propia** (R9a) — cada carga de
  ...
  aprobado en R8/§3 de `SPEC-ARQ-0309202601`.
...
### 5.3 Corrección: API HTTP de GitHub en vez de `git`/`gh` (R9c, 2026-09-22)

`commits_recientes/1` y `runs_actualizar_sistema/0`
(`PropagacionContext`) dejan de shellear `git`/`gh` — pasan a usar
`Req` (librería HTTP estándar de este proyecto) contra la API REST de
GitHub directo:

- Commits: `GET https://api.github.com/repos/prettycore-13/metadata_stack/commits?sha=main&per_page=<limite>`
  — cada elemento trae `sha`, `commit.message` (primera línea = mensaje
  corto), `commit.author.name`/`commit.author.date`. `hash_corto` se
  calcula con `String.slice(sha, 0, 7)` (mismo largo que `%h` de git y
  que `GIT_SHA_CORTO` del footer, R11).
- Runs: `GET https://api.github.com/repos/prettycore-13/metadata_stack/actions/workflows/actualizar-sistema.yml/runs?per_page=100`
  — campos `head_sha`/`status`/`conclusion`/`display_title`/`created_at`/`updated_at`,
  agrupados por `head_sha` igual que antes. `per_page=100` (máximo de
  la API por página) en vez del `--limit 200` anterior — suficiente
  para calzar contra los últimos 30 commits que muestra la pantalla por
  default; no se pagina más para no complicar el caso real.
- Headers: `authorization: Bearer <token>`,
  `accept: application/vnd.github+json`,
  `x-github-api-version: 2022-11-28`.
- Token: `System.get_env("GITHUB_TOKEN_LECTURA")`, leído en
  `config/runtime.exs` fuera del bloque `if config_env() == :prod`
  (mismo criterio que `version_footer`, R11 — vale en cualquier
  ambiente, incluido dev local) y expuesto como
  `Application.get_env(:metadata_app, :github_token_lectura)`. **Sin
  `raise` si falta** (a diferencia de `DB_HOSTNAME_PSQL`/`SECRET_KEY_BASE`):
  esta pantalla es una funcionalidad más, no algo sin lo cual el
  release no debe arrancar — un ambiente sin el token configurado sigue
  sirviendo el resto de la app, solo esta pantalla muestra el error de
  R9c en vez de la línea de tiempo. Scope del token: solo lectura
  (`contents:read` + `actions:read`) — nunca uno con permiso de
  disparar workflows (ver "Fuera de alcance" en `01.requirements.md`).
- `linea_de_tiempo/2` cambia su firma: de devolver la lista directo a
  `{:ok, [commit]} | {:error, mensaje}` — el error (token ausente, o
  cualquier respuesta HTTP != 200) llega tal cual hasta la UI
  (`PropagacionLive` ya tiene `@error_carga` para esto, ver §5.2) en
  vez de perderse como `[]` silencioso.
- `posiciones_por_hash/1` (SSH vía `Estado.consultar_todo/0`) no
  cambia — nunca dependió de `git`/`gh`.
- `migraciones_entre/2` (usado solo por el rollback de base de datos,
  R10a/R10b) tampoco cambia — sigue shelleando `git` local, correcto
  porque ya está fuera de alcance de esta corrección (mismo camino roto
  que R8/R10 en un pod desplegado, ver nota de `01.requirements.md`).
...
## 7. Versión en pie de página (R11-R11a)

**Origen del dato**: el hash corto y la fecha del commit se capturan en
`.github/workflows/ci.yml`, en el job `build-image` (mismo job que ya
tagea `ghcr.io/${{ env.IMAGE_NAME }}:${{ github.sha }}`, línea ~165) —
un step nuevo ANTES del `docker/build-push-action@v6`, sobre el
checkout que ese job ya tiene:
...
Funciona con el checkout shallow (`depth=1`) default de
`actions/checkout` — ambos comandos solo necesitan metadata del commit
`HEAD`, no el historial completo.

`docker/build-push-action@v6` gana `build-args`:
...
**`Dockerfile`**: declarar en el stage `runner` final (no en `builder`
— este dato solo hace falta en tiempo de ejecución, no durante la
compilación):
...
Quedan disponibles como variables de entorno del contenedor corriendo,
mismo mecanismo que cualquier otra `ENV` de runtime que ya lee
`config/runtime.exs`.

**`config/runtime.exs`**: mismo patrón que `:nombre_empresa` (línea 27
— fuera del bloque `if config_env() == :prod`, vale para todos los
ambientes, así también se puede forzar en dev exportando la env a mano
si hace falta probarlo sin build real):
...
Sin esas env (dev local, `mix test`, cualquier corrida fuera de la
imagen de release) ambos campos quedan `nil` — el footer simplemente no
muestra nada extra ahí, correcto: no hay ninguna imagen de release
corriendo en esos casos.

**`MetadataAppWeb.MenuLayout.sidebar/1`**: mismo patrón que
`:bpb_habilitado` (línea 41, `Application.get_env/3` dentro del pipe de
`assign`):
...
Y en el `.heex` (`menu_layout.ex`, línea 362), junto al
...
`:if` cubre el caso dev/test (R11a: nunca mostrar un valor inventado o
desactualizado — si no hay dato real, no se muestra nada).

## 8. Resumen de módulos nuevos/tocados
...
| `.github/workflows/ci.yml` | step nuevo (captura hash+fecha) + `build-args` (§7) |
| `Dockerfile` | `ARG`/`ENV` `GIT_SHA_CORTO`/`GIT_SHA_FECHA` en stage `runner` (§7) |
| `config/runtime.exs` | `:version_footer` (§7) |
| `lib/metadata_app_web/components/menu_layout.ex` | assign + span en pie de página (§7) |

## 9. Fuera de alcance (igual que `01.requirements.md` §10)

Alta de sistema nuevo (`SPEC-ARQ-0309202601`), propagación de
