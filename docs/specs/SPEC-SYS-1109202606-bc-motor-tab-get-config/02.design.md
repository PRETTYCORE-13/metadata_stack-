...
el camino real sigue siendo la Ficha). Mismas clases `checkbox
checkbox-sm` (daisyUI) que ya usa `<.input type="checkbox">` en
`core_components.ex`, sin el wrapper de formulario (`<label>`, hidden
input, `<.error>`) que ese componente trae — acá no hay ningún form
detrás, es de solo lectura.
