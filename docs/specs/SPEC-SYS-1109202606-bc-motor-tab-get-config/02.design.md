...
**Documento:** Design · **Fase:** ✅ aprobada (2026-09-11) — con 1 fix real (§5, R14): código muerto eliminado a pedido explícito del usuario, ver `03.tasks.md`. **2026-09-12**: §5 completa ("Filtros por default") ELIMINADA del código a pedido explícito del usuario — ver §5 reescrita y `03.tasks.md` Grupo E. **2026-09-22**: §6 nueva (R19, checkbox visual para columnas booleanas) — pendiente de codificar, ver `03.tasks.md` Grupo F.
...
## 2. Tabla unificada de columnas (R2-R7)
...
ya tenía `CatalogoLive` antes de unificar esta tabla. El orden final
...
todavía en esta tabla, no cambian de posición relativa entre sí,
quedan al final en su orden natural).

**Reordenar (R5)**: mismo hook `ListaOrdenable` que la tabla de
...
cual dentro de esta misma fila de la tabla (`campo_param` arma el
...
eliminó su rama de UI el 2026-09-11 (R14, ver `03.tasks.md` Grupo D).
...
## 6. Renderizado de columnas booleanas (R19, a pedido explícito, 2026-09-22)

`celda_body/1` (`catalogo_live.ex:2007-2019`), la cláusula
`tipo_columna: :negocio` — hoy interpola `{formatear_celda(@valor,
@col.columna.schema_context_properties)}` directo, y `formatear_celda/2`
no tiene ninguna cláusula para `"tipo" => "boolean"` — cae en el
catch-all (`defp formatear_celda(valor, _props), do: valor`,
`:2381`), así que HEEx interpola el átomo `true`/`false` crudo
(`Phoenix.HTML.Safe` para `Atom` → `Atom.to_string/1`) — de ahí el
texto literal "true"/"false" que se ve hoy.

**Cambio**: en `celda_body/1`, antes de interpolar, chequear
`@col.columna.schema_context_properties["tipo"] == "boolean"` — si es
así, renderizar un checkbox deshabilitado en vez de llamar
`formatear_celda/2`:

```heex
<td data-col={@col.clave} class={[...]}>
  <input
    :if={@col.columna.schema_context_properties["tipo"] == "boolean"}
    type="checkbox"
    checked={@valor == true}
    disabled
    class="rounded border-gray-300 accent-blue-600"
  />
  {if @col.columna.schema_context_properties["tipo"] != "boolean", do: formatear_celda(@valor, @col.columna.schema_context_properties)}
</td>
```

`checked={@valor == true}` (no `checked={@valor}` a secas) — un campo
booleano sin valor real (`nil`, nunca seteado) queda visualmente
desmarcado, igual que `false`, sin distinguir "nunca configurado" de
"configurado en false" — mismo criterio que ya usa el resto de la
tabla para valores vacíos (no hay una tercera marca visual para "sin
dato" en una columna booleana).

`disabled` a propósito — R19 es un parseo puramente visual, nunca un
control editable desde la tabla (para editar un booleano de negocio,
el camino real sigue siendo la Ficha).

**Corregido tras verificar en real (2026-09-22)**: la primera versión
usaba `class="checkbox checkbox-sm"` (daisyUI, mismas clases que
`<.input type="checkbox">` de `core_components.ex`) — el checkbox
quedaba invisible en pantalla (confirmado con capturas reales del
usuario contra un catálogo real, `pty_sat_usocfdi`, con datos
`true`/`false` correctos y metadata correcta — la celda se veía
completamente en blanco). Este proyecto evita daisyUI a propósito
(`AGENTS.md`: "Always manually write your own tailwind-based
components instead of using daisyUI"). Corregido a las mismas clases
planas de Tailwind que ya usa, en esta misma tabla, el checkbox de
selección de fila (`catalogo_live.ex:1688`, `rounded border-gray-300` —
sin `focus:ring-purple-500`, que ese otro sí trae porque es
interactivo y este es de solo lectura) — patrón ya confirmado visible
en la misma página.

**Segundo hallazgo real (mismo día)**: pedido explícito del usuario de
pintarlo azul — `class="... text-blue-600"` compiló bien (confirmado
en `priv/static/assets/css/app.css`) pero el checkbox seguía viéndose
con el color nativo del navegador (negro), sin cambio visual real.
Causa: `text-{color}` solo controla el color de un checkbox NATIVO si
el plugin `@tailwindcss/forms` está cargado (restyla
`input[type=checkbox]` para que el color de texto pase a ser su
`accent-color`) — este proyecto NO lo tiene. Confirmado que ESE
también era el motivo por el que el checkbox de selección de fila
(`:1688`, mismo `text-purple-600`) nunca se vio morado tampoco, nadie
lo había notado hasta ahora. Corregido a `accent-blue-600` — utilidad
propia de Tailwind (mapea directo a la propiedad CSS `accent-color`,
sin necesitar el plugin de forms), soportada nativamente por
`input[type=checkbox]`/`radio` en navegadores modernos. Azul, distinto
del morado del checkbox de selección, para no confundir visualmente
"este es un dato del registro" con "este registro está seleccionado".

**Columnas de control**: ninguna de las 9 (`@campos_control`,
`bc_motor_live.ex:54-64`) es de tipo booleano hoy — R19 no necesita
tocar `celda_body/1` para `tipo_columna: :control` mientras eso siga
así. Si algún día se agrega una columna de control booleana, el mismo
chequeo aplicaría ahí, pero queda fuera de esta spec hasta que exista
un caso real (no se agrega código muerto para un caso hipotético).

## 7. Fuera de alcance

Igual que `01.requirements.md` §7.
