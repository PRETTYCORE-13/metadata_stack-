...
así, renderizar un indicador visual tipo checkbox en vez de llamar
`formatear_celda/2`:
...
`@valor == true` (no `@valor` a secas) — un campo booleano sin valor
real (`nil`, nunca seteado) queda visualmente desmarcado, igual que
`false`, sin distinguir "nunca configurado" de "configurado en false"
— mismo criterio que ya usa el resto de la tabla para valores vacíos
(no hay una tercera marca visual para "sin dato" en una columna
booleana).

**Por qué un `<div>` y no un `<input type="checkbox" disabled>`
real** — de solo lectura a propósito (para editar un booleano de
negocio, el camino real sigue siendo la Ficha), pero llegar acá costó
dos vueltas reales, verificando en vivo contra un catálogo real
(`pty_sat_usocfdi`, campo `es_moral`, con capturas del usuario) porque
cada intento con un `<input>` real fallaba de una forma DISTINTA:

1. `class="checkbox checkbox-sm"` (daisyUI, mismas clases que
   `<.input type="checkbox">` de `core_components.ex`) — invisible en
   pantalla. Este proyecto evita daisyUI a propósito (`AGENTS.md`:
   "Always manually write your own tailwind-based components instead
   of using daisyUI").
2. `class="rounded border-gray-300 accent-blue-600"` (o `text-blue-600`
   antes) — visible, pero el color de acento NUNCA cambiaba, seguía
   negro nativo del navegador sin importar la clase. Causa real: el
   color de acento de un `input[type=checkbox]` **disabled** queda
   apagado/gris por la hoja de estilo nativa de Chrome/Edge — ninguna
   clase CSS lo pisa mientras el elemento esté disabled (confirmado
   real: ni el checkbox de selección de fila de esta misma tabla,
   `:1688`, mismo `text-purple-600`, se veía morado nunca — nadie lo
   había notado hasta ahora porque nunca está disabled).

Un `<div>` con un ícono adentro (Material Symbols, mismo sistema de
íconos que ya usa el resto de esta tabla) da control total del color
sin pelear con ninguna limitación de un control nativo — mismo
criterio de "escribí tu propio componente" que ya rige para daisyUI.
Azul, distinto del morado del checkbox de selección, para no confundir
visualmente "este es un dato del registro" con "este registro está
seleccionado".
