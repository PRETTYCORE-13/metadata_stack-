...
**Bug real encontrado y corregido (2026-09-17)**: `bc-deploy.yml` (el camino de `mix motor.publicar`, el que se usa día a día) NO tenía el mismo paso de "restaurar todos los bc-*" que sí tiene `ci.yml` desde el párrafo de arriba — cada publicación armaba una imagen con SOLO el catálogo que se estaba publicando en ese momento, así que publicar cualquier BC nuevo dejaba sin código compilado (aunque con la metadata todavía en la base, por eso se veía "vacío" en vez de roto) a TODOS los BCs publicados antes. Corregido replicando el mismo paso en `bc-deploy.yml`.

**Pendiente para retomar (2026-09-17, a pedido explícito, no urgente todavía)**: el fix de arriba hace que CADA publicación (sea 1 catálogo o 1000 ya publicados) baje y recompile el paquete completo desde cero — correcto y simple, pero no escala indefinidamente. El día que el número de BCs reales sea grande y esto se sienta lento, la solución de fondo es dejar de reconstruir la imagen entera desde cero en cada publish (ej. que los BCs vivan en un lugar que el pod ya tiene siempre disponible en vez de "armar todo de nuevo" — layering de imagen incremental, o un volumen/artifact store separado del build). Sin diseñar todavía.
...
**Pendiente, sin diseñar todavía**:
- Una pantalla en `/sysadmin` (visible solo para el sysadmin del sistema) donde configurar host/puerto/usuario/contraseña de SMTP — mismo patrón ya probado en `/sysadmin/credenciales` (`MetadataApp.Integraciones.Credencial`, campo `api_key_nuevo` virtual que nunca se precarga) o en `/sysadmin/ambientes-deploy`.
