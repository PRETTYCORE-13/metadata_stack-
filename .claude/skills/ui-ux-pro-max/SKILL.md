...
2	Touch & Interaction	CRITICAL	`ux`	Min size 44×44px, 8px+ spacing, Loading feedback	Reliance on hover only, Instant state changes (0ms), Adding a new fixed/floating element (FAB, toggle, chat widget, cookie banner) without auditing existing `position: fixed` UI for the same viewport corner
...
Floating buttons overlap on mobile	Before adding any `position: fixed` element (FAB, footer toggle, chat widget, cookie banner, "scroll to top"), grep the codebase for existing `fixed` rules combined with `bottom`/`top`/`left`/`right` and check their coordinates + z-index. Two floating elements added independently (often in different files/components, at different times) will silently stack in the same corner — this is easy to miss in code review since neither diff shows the other element, and only shows up visually on a real small viewport. Found live: a new mobile FAB (`fixed bottom-5 right-5`) landed exactly on top of a pre-existing footer-toggle button (`fixed bottom:12px right:12px`) — both rendered, but overlapping, and it only surfaced once a user sent a phone screenshot.
