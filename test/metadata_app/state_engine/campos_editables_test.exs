defmodule MetadataApp.StateEngine.CamposEditablesTest do
  use MetadataApp.DataCase, async: true

  alias MetadataApp.StateEngine
  alias MetadataApp.CatalogoGenerico
  alias MetadataApp.MetaSchema.{Header, Detail, Estado}
  ...
  describe "campos_editables/2 — catálogo SIN motor de estados" do
    test "devuelve todos los campos, sin restringir nada" do
      ...
      campos = StateEngine.campos_editables("pty_canal", nil)
  ...
  describe "campos_editables/2 — catálogo CON motor de estados" do
    test "solo devuelve los campos con editable_en para el estado actual" do
      ...
      assert Enum.sort(StateEngine.campos_editables("pty_clientes", nuevo.id)) == [
               "pty_clientes_nombre"
             ]

      assert Enum.sort(StateEngine.campos_editables("pty_clientes", activo.id)) == [
               "pty_clientes_edad",
               "pty_clientes_nombre"
             ]
    end
    ...
    test "estado_id nil no habilita ningún campo" do
      ...
      assert StateEngine.campos_editables("pty_clientes", nil) == []
    end
  end

  describe "estado_inicial/1" do
    test "nil si el catálogo no adoptó el motor" do
      assert StateEngine.estado_inicial("pty_canal") == nil
    end
    ...
    test "devuelve el estado marcado es_inicial: true" do
      ...
      assert StateEngine.estado_inicial("pty_clientes").id == inicial.id
    end
  end
  ...
  describe "CatalogoGenerico.crear/2 — asignación automática del estado inicial" do
    test "catálogo sin motor de estados: estado_id sigue en nil" do
      {:ok, canal} =
        CatalogoGenerico.crear(MetadataApp.Catalogos.PtyCanal, %{
